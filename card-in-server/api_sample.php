...
class cardApi{
	...
	public function get_device_list($student_no,$token=""){
		...
		while ($row = $this->db->fetch_array($query)) {
			$item["building"] = $this->config["build_map"][substr($row["device_id"],0,1)];
			$item["floor"] = substr($row["device_id"],1,1);
			$item["room"] = substr($row["device_id"],2,3);
			$item["deviceName"] = $this->config["build_device_name"][substr($row["device_id"],5,1)];
			...
			$item["deviceType"] = $this->config["build_device_type"][substr($row["device_id"],5,1)];
			$item["deviceIcon"] = $this->config["build_device_icon"][substr($row["device_id"],5,1)];
			$devices[] = $item;
		}
		$result["resp_code"] = "0";
		$result["resp_desc"] = "";
	...
	//开淋浴房
	public function open_shower($student_no,$device_id, $time, $delay_open, $delay_close,$token){
	...
	//关淋浴房
	public function close_shower($student_no,$device_id,$token){
		$end_time = time();
		$this->operate_device_with_fee($student_no,$device_id,"CLOSE",0,"111111",$token,0,$end_time);
	}
	...
	//开洗衣机
	public function close_washer($student_no,$device_id,$token){
		$end_time = time();
	...
	private function operate_device_with_fee($student_no,$device_id,$operate="OPEN",$fee,$password="",$token="",$begin_time=0,$end_time=0){
		...
		if($result == 2){
			...
			if($operate == "OPEN"){
				...
				$values = array("flag"=>"0","begin_time"=>$begin_time,"pre_end_time"=>$end_time,"end_time"=>time(),"fee_flag"=>0);
				$query = $this->db->update("devices",$values,$condition);
				if($query){
					$result_msg = "关闭成功，数据更新成功";
				}else{
					$result_msg = "关闭成功，数据库更新失败";
				}
